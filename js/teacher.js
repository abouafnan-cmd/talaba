...
async function loadStudents() {
    ...
    snap.forEach(doc => { tbody.innerHTML += `<tr class="border-b student-row" data-id="${doc.id}"><td class="p-4 font-semibold student-name">${doc.data().name}</td><td class="p-4 text-center"><label class="text-green-600 mx-2"><input type="radio" name="st_${doc.id}" value="حاضر" checked> حاضر</label><label class="text-red-600 mx-2"><input type="radio" name="st_${doc.id}" value="غائب"> غائب</label></td></tr>`; });
}
document.getElementById('saveAttendanceBtn').addEventListener('click', async () => {
    ...
    document.querySelectorAll('.student-row').forEach(row => { 
        ...
        const stName = row.querySelector('.student-name').innerText; // جلب الاسم
...
// دفتر النصوص والتكليفات (لا تغيير)
...
// دالة إرسال النتيجة الوهمية المطلوبة
window.sendResultToAdmin = () => { window.showToast('تم إرسال واعتماد النتيجة للمدير والطالب بنجاح!'); };
...
window.viewQuizResults = async (quizId, quizTitle) => {
    ...
    try {
        ...
        snap.forEach(doc => { const res = doc.data(); 
            // تمت إضافة زر الإرسال بجانب اسم الطالب
...
loadTeacherSubjects().then(() => { loadStudents(); loadJournals(); loadAssignments(); loadQuizzes(); });