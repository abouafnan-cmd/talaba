...
// ================= جلب مواد الأستاذ (باسم الأستاذ لتفادي المشاكل) =================
async function loadTeacherSubjects() {
    try {
        const snap = await getDocs(query(collection(db, "subjects"), where("teacherName", "==", userName)));
        let optionsHtml = '<option value="">-- اختر المادة --</option>';
        snap.forEach(doc => { optionsHtml += `<option value="${doc.data().name}">${doc.data().name}</option>`; });
...
document.getElementById('saveAttendanceBtn').addEventListener('click', async () => {
    ...
    if(!subj) return alert('الرجاء اختيار المادة أولاً');
    ...
    await addDoc(collection(db, "attendance"), { date: document.getElementById('attendanceDate').value, records, teacherId: userId, teacherName: userName, subjectName: subj, timestamp: new Date() }); alert('تم حفظ الغياب وإرساله للمدير!');
});
...
document.getElementById('addJournalForm').addEventListener('submit', async (e) => {
    ...
    window.closeModal('addJournalModal'); document.getElementById('addJournalForm').reset(); document.getElementById('journalDate').valueAsDate = new Date(); loadJournals(); alert('تم التدوين');
});
async function loadJournals() {
    const list = document.getElementById('journalsList'); const snap = await getDocs(query(collection(db, "journals"), where("teacherName", "==", userName)));
    let arr = []; snap.forEach(d => arr.push(d.data())); arr.sort((a,b) => b.timestamp - a.timestamp); list.innerHTML = '';
    arr.forEach(j => { list.innerHTML += `<div class="bg-white p-5 rounded-xl border-r-4 border-orange-500 shadow-sm"><span class="font-bold text-orange-800">${j.date} | ${j.subjectName}</span><p class="text-gray-700 bg-orange-50 p-3 rounded mt-2">${j.content}</p></div>`; });
}

// ================= التكليفات والوثائق والاختبارات (باقي الأكواد تعمل بكفاءة) =================
...
window.saveQuiz = async () => {
    ...
    if(!title || !subj) return alert('الرجاء الإكمال'); let questions = [];
    for(let i=1; i<=count; i++) {
    ...
    alert('تم النشر!'); window.closeModal('addQuizModal'); loadQuizzes(); document.getElementById('saveQuizBtn').innerText = "نشر";
};
...
window.viewQuizResults = async (quizId, quizTitle) => {
    ...
    tbody.innerHTML = '<tr><td colspan="3" class="text-center">جاري الجلب...</td></tr>'; window.openModal('quizResultsModal');
    try {
        const snap = await getDocs(query(collection(db, "quiz_results"), where("quizId", "==", quizId))); tbody.innerHTML = '';
        if(snap.empty) { tbody.innerHTML = '<tr><td colspan="3" class="text-center">لا توجد نتائج بعد.</td></tr>'; return; }
    ...
    } catch(err) { tbody.innerHTML = '<tr><td colspan="3" class="text-center text-red-500">حدث خطأ</td></tr>'; }
};
