...
window.deleteRecord = async (col, id, type) => { if(confirm('تأكيد الحذف نهائياً؟')) { try { await deleteDoc(doc(db, col, id)); window.showToast('تم الحذف'); if(type==='users') loadUsers(); if(type==='cohorts') loadCohorts(); if(type==='subjects') loadSubjects(); if(type==='announcements') loadAnnouncements(); if(type==='assignments') loadAdminAssignments(); if(type==='quizzes') loadAdminQuizzes(); } catch(e) { window.showToast('خطأ', 'error'); } } };

// دوال جلب الفلاتر الشاملة
...
// الدوال الأساسية (Users, Cohorts, Subjects, Announcements) - نفس الأكواد السابقة لضمان عملها
async function loadUsers() { const tbody = document.getElementById('usersTableBody'); const snap = await getDocs(query(collection(db, "users"), orderBy("timestamp", "desc"))); tbody.innerHTML = ''; snap.forEach(d => { const u = d.data(); tbody.innerHTML += `<tr class="border-b"><td class="p-4 font-semibold">${u.name}</td><td class="p-4">${u.role}</td><td class="p-4 text-center"><button onclick="window.editUser('${d.id}','${u.name}','${u.phone}','${u.password}','${u.role}')" class="px-2 py-1 bg-blue-500 text-white rounded text-xs shadow">تعديل</button> <button onclick="window.deleteRecord('users', '${d.id}', 'users')" class="px-2 py-1 bg-red-500 text-white rounded text-xs shadow">حذف</button></td></tr>`; }); }
window.editUser = (id, name, phone, pass, role) => { document.getElementById('editUserId').value=id; document.getElementById('newName').value=name; document.getElementById('newPhone').value=phone; document.getElementById('newPassword').value=pass; document.getElementById('newRole').value=role; window.openModal('addUserModal'); };
...
async function loadCohorts() { const tbody = document.getElementById('cohortsTableBody'); const snap = await getDocs(query(collection(db, "cohorts"), orderBy("timestamp", "desc"))); tbody.innerHTML = ''; snap.forEach(d => { tbody.innerHTML += `<tr class="border-b"><td class="p-4 font-bold">${d.data().name}</td><td class="p-4 text-center"><button onclick="window.deleteRecord('cohorts', '${d.id}', 'cohorts')" class="px-2 py-1 bg-red-500 text-white rounded text-xs shadow">حذف</button></td></tr>`; }); }
document.getElementById('addCohortForm').addEventListener('submit', async (e) => { e.preventDefault(); await addDoc(collection(db, "cohorts"), { name: document.getElementById('newCohortName').value, timestamp: new Date() }); window.closeModal('addCohortModal'); loadCohorts(); });
...
// ================= الفلاتر للغياب، الدفتر، التكليفات والاختبارات =================
...
window.loadAdminJournals = async () => {
    const fDate = document.getElementById('filterJournDate').value; const fSubj = document.getElementById('filterJournSubj').value;
    const list = document.getElementById('adminJournalsList'); const snap = await getDocs(query(collection(db, "journals"), orderBy("timestamp", "desc"))); list.innerHTML = '';
    snap.forEach(d => { const j = d.data(); if((fDate && j.date !== fDate) || (fSubj && j.subjectName !== fSubj)) return; list.innerHTML += `<div class="bg-white p-5 rounded-xl border-r-4 border-orange-500 shadow-sm"><span class="bg-orange-100 px-3 py-1 rounded font-bold">${j.date}</span><span class="font-semibold ml-4">👨‍🏫 ${j.teacherName} | 📚 ${j.subjectName}</span><p class="mt-2 bg-gray-50 p-3 rounded">${j.content}</p></div>`; });
};

window.loadAdminAssignments = async () => {
    const fSubj = document.getElementById('filterAssSubj').value;
    ...
    snap.forEach(d => { const data = d.data(); if(fSubj && data.subjectName !== fSubj) return; const date = data.timestamp ? data.timestamp.toDate().toLocaleDateString('en-GB') : ''; tbody.innerHTML += `<tr class="border-b"><td class="p-4">${data.subjectName}</td><td class="p-4 font-bold">${data.title}</td><td class="p-4">${date}</td><td class="p-4 text-center"><button onclick="window.deleteRecord('assignments', '${d.id}', 'assignments')" class="bg-red-500 text-white px-3 py-1 rounded">حذف</button></td></tr>`; });
};

window.loadAdminQuizzes = async () => {
    const fSubj = document.getElementById('filterQuizSubj').value;
    ...
    snap.forEach(d => { const data = d.data(); if(fSubj && data.subjectName !== fSubj) return; const date = data.timestamp ? data.timestamp.toDate().toLocaleDateString('en-GB') : ''; tbody.innerHTML += `<tr class="border-b"><td class="p-4">${data.subjectName}</td><td class="p-4 font-bold">${data.title}</td><td class="p-4">${date}</td><td class="p-4 text-center space-x-2 space-x-reverse"><button onclick="window.viewQuizResults('${d.id}', '${data.title.replace(/'/g, "\\'")}')" class="bg-purple-500 text-white px-3 py-1 rounded">النتائج</button><button onclick="window.deleteRecord('quizzes', '${d.id}', 'quizzes')" class="bg-red-500 text-white px-3 py-1 rounded">حذف</button></td></tr>`; });
};
...
window.downloadPDF = (containerId, filename) => {
    ...
    element.querySelectorAll('.pdf-title').forEach(el => el.classList.remove('hidden')); 
...
async function init() {
    ...
    const tSnap = await getDocs(query(collection(db, "users"), where("role", "==", "أستاذ"))); const tSel = document.getElementById('selectTeacher'); tSel.innerHTML = ''; tSnap.forEach(d => tSel.innerHTML += `<option value="${d.id}">${d.data().name}</option>`);
    loadUsers(); loadCohorts(); loadSubjects(); loadAnnouncements(); window.loadAdminAttendance(); window.loadAdminJournals(); window.loadAdminAssignments(); window.loadAdminQuizzes();
}
init();